...
$fromEmailAddress = "youraccount@gmail.com"
$fromEmailPassword = "yourpasswordhere"
$toEmailAddress = "someone@your.org"
...
Functions to get previous and current IP addresses, as well as 
the last day script ran to support the $emailOnceADay option.

-----------------------------------------------------------------------------#>

function Get-LastIPv4 ($Path)
{
  # Get the IP Address from the last time the script ran
  $lastIPv4File = "$Path\LastIPv4.txt"
  
  if ((Test-Path $lastIPv4File) -eq $false)
  {
  ...
  else
  {
    ...
    $lastIPv4 = Get-Content $lastIPv4File -Raw
    $lastIPv4 = $lastIPv4 -replace "`t|`r|`n", ""
  }

  return $lastIPv4
}

function Get-LastIPv6 ($Path)
{
  # Get the IP Address from the last time the script ran
  $lastIPv6File = "$Path\LastIPv6.txt"
  
  if ((Test-Path $lastIPv6File) -eq $false)
  {
    # If the file doesn't exist, just set to empty string
    $lastIPv6 = ''
  }
  ...
  {
    # Get the IP Address from the last run, and strip out any special characters
    $lastIPv6 = Get-Content $lastIPv6File -Raw
    $lastIPv6 = $lastIPv6 -replace "`t|`r|`n", ""
  }
...
{
  # Get external IPv4 Address - added by nanovak
  $extIPv4 = Invoke-RestMethod ipv4.whatip.me/?json
  $externalIPv4 = $extIPv4.IP
...
{
  # Get external IPv6 Address - added by nanovak
  $extIPv6 = Invoke-RestMethod ipv6.whatip.me/?json
  $externalIPv6 = $extIPv6.IP
...
# Get the current IP addresses and the addys from the last time the script ran
$lastIPv4 = Get-LastIPv4 -Path $path
$lastIPv6 = Get-LastIPv6 -Path $path
$externalIPv4 = Get-ExternalIPv4
$externalIPv6 = Get-ExternalIPv6
...
if ($externalIPv4 -ne $lastIPv4) 
{ $sendEmail = $true }

if ($externalIPv6 -ne $lastIPv6) 
{ $sendEmail = $true }
...
#
# The following section creates and sends the email and... 
#   Updates the last IP and run date
#

# Send the email if the checks said we need to
if ( $sendEmail -eq $true   )
{
  $subject = "HOME IP CHANGE: $externalIPv4 & $externalIPv6"
  
  $body = @"
As of $(Get-Date -Format "yyyy-MMM-dd HH:mm") a public IP change was detected at home.

Current IPv4:`t$externalIPv4
Prior IPv4:`t$lastIPv4
---
Current IPv6:`t$externalIPv6
Prior IPv6:`t$lastIPv6
"@
  ...
  # Update the last IP address files
  $lastIPv4File = "$Path\LastIPv4.txt"
  $lastIPv6File = "$Path\LastIPv6.txt"
  Set-Content -Path $lastIPv4File -Value $externalIPv4 -Force
  Set-Content -Path $lastIPv6File -Value $externalIPv6 -Force
...
# Printing these out to the window for debugging whan manually running the script
"  Current IPv4: $externalIPv4"
"  Current IPv6: $externalIPv6"
